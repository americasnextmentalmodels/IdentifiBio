...
class ChangeEmailController: UIViewController{
    ...
    let newPasswordTextField: UITextField = {
        ...
        //For testing purposes, remove for development build.
        tf.attributedPlaceholder = NSAttributedString(string: "New Email", attributes: [NSAttributedStringKey.foregroundColor: UIColor(r: 230, g: 230, b: 230)])
    ...
    let confirmNewPasswordTextField: UITextField = {
        ...
        //For testing purposes, remove for development build.
        tf.attributedPlaceholder = NSAttributedString(string: "Confirm New Email", attributes: [NSAttributedStringKey.foregroundColor: UIColor(r: 230, g: 230, b: 230)])
        ...
        user?.reauthenticate(with: credential) { error in
            if error != nil {
                ...
                if (self.comparePasswords(pw1: self.newPasswordTextField.text!, pw2: self.confirmNewPasswordTextField.text!)){
                    Auth.auth().currentUser?.updateEmail(to: self.newPasswordTextField.text!) { (error) in
                        if (error != nil) {
                            self.errorLabel.text = "ERROR: " + (error?.localizedDescription)!
                            return
                        }
                        ...
                        self.errorLabel.text = "Successfully changed email"
                        
                        // update Database reference to email
                        guard let uid = Auth.auth().currentUser?.uid else {
                            //uid not available so do nothing
                            return
                        }
                        let values = ["email": self.newPasswordTextField.text] as [String : Any]
                        ...
                        print("User changed email")
                    }
                }
                else{
                    self.errorLabel.text = "Emails do not match. Try again"
                    return
                }
            }
        }
    ...
    override func viewWillAppear(_ animated: Bool){
        ...
        navigationController?.navigationBar.topItem?.title = "Change Email"
    ...
    func setupInputsContainerView(){
        ...
        errorLabel.topAnchor.constraint(equalTo: ChangeEmailButton.bottomAnchor, constant: 15).isActive = true
    ...
    func setupChangePasswordButton(){
        ChangeEmailButton.centerXAnchor.constraint(equalTo: inputsContainerView.centerXAnchor).isActive = true
        ChangeEmailButton.topAnchor.constraint(equalTo: confirmNewPasswordTextField.bottomAnchor, constant: 30).isActive = true
        ChangeEmailButton.widthAnchor.constraint(equalTo: inputsContainerView.widthAnchor, multiplier: 1/2).isActive = true
        ChangeEmailButton.heightAnchor.constraint(equalToConstant: 45).isActive = true
    }
